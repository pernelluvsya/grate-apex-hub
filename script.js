(function () {
  ...
  var THEMES = [
    ...
    { id: "lavender", icon: "💜", name: "Lavender Haze", desc: "Soft dreamy purple.", unlock: 18 },
    { id: "rainbow", icon: "🌈", name: "Rainbow", desc: "Full colour party mode.", unlock: 20 }
  ];
  ...
  function load() {
    ...
    if (typeof mem.ansStreak !== "number") mem.ansStreak = 0;
    if (typeof mem.wrongStreak !== "number") mem.wrongStreak = 0;
  ...
  function initFirebaseAuth() {
    ...
    window.GAFirebase.onAuthChange(function (user) {
      if (user) {
        gUser = user; setGoogleBtn("signedin");
        var d = load();
  ...
  /* ---------- derived ---------- */
  function totals() {
    var d = load(), a = 0, c = 0; HUBS.forEach(function (k) { a += d.subjects[k].answered; c += d.subjects[k].correct; });
    return { ans: a, cor: c, acc: a ? Math.round(c / a * 100) : null };
  }
  function rankFor(xp) {
    var lvl = 1 + Math.floor(xp / 150); var title = RANKS[0][1];
  ...
  function streak() {
    var d = load(), days = d.days, t = todayStr();
  ...
  function sprint() {
    var start = new Date(SPRINT_START_DATE + "T00:00:00"); var end = new Date(EXAM_DATE + "T00:00:00"); var now = new Date(todayStr() + "T00:00:00");
  ...
  function grant(id) {
    var d = load(); if (d.badges[id]) return false; var b = BADGES.filter(function (x) { return x.id === id; })[0];
    d.badges[id] = Date.now(); save(); if (b) toast("🏅 Badge unlocked: " + b.t); return true;
  }
  function checkGolden() {
    var d = load(); var all = Object.keys(d.badges).length >= BADGES.length;
    document.documentElement.setAttribute("data-golden", all ? "1" : "0");
    if (all && !d.goldenCelebrated) {
      d.goldenCelebrated = true; save();
      setTimeout(function () { confetti(); chime(); toast("🏆 All achievements unlocked — GOLDEN status! ✨", 6500); }, 500);
    }
  }
  function checkBadges() {
    var d = load(), T = totals(); var h = new Date().getHours();
  ...
  function chime() {
    try {
      ...
      [523, 659, 784, 1047].forEach(function (f, i) {
        var o = actx.createOscillator(), g = actx.createGain(); o.type = "triangle"; o.frequency.value = f;
        o.connect(g); g.connect(actx.destination); var t = actx.currentTime + i * 0.09; g.gain.setValueAtTime(0.0001, t);
        g.gain.exponentialRampToValueAtTime(0.22, t + 0.02); g.gain.exponentialRampToValueAtTime(0.0001, t + 0.28); o.start(t); o.stop(t + 0.3);
      });
    } catch (e) { }
  }
  function confetti() {
    var cv = document.getElementById("confetti"); cv.style.display = "block";
    ...
    (function frame() {
      ...
      P.forEach(function (p) {
        p.y += p.vy; p.x += p.vx; p.rot += p.vr; if (p.y < cv.height + 20) alive = true;
  ...
  function recordQuiz(key, p) {
    ...
    if (p.topicStats) {
      for (var tk in p.topicStats) {
        var ts = p.topicStats[tk] || {}; if (!s.topics[tk]) s.topics[tk] = { answered: 0, correct: 0 };
  ...
  function maybeLevelUp(oldXp, newXp) {
    var ol = rankFor(oldXp), nl = rankFor(newXp);
    if (nl.lvl > ol.lvl) {
      setTimeout(function () {
        confetti(); chime();
        toast(nl.title !== ol.title ? ("⬆️ New rank: " + nl.title + " — Level " + nl.lvl + "!") : ("⬆️ Level up! Apex Level " + nl.lvl), 4200);
      }, 400);
    }
  }
  var FACTS = [
  ...
  function cardHTML(key) {
    var m = META[key]; return '<button class="card" data-hub="' + key + '" onclick="GA.open(\'' + key + '\')" aria-label="Open ' + m.name + ' hub">'
  ...
  function renderCards() {
    var g = document.getElementById("cardGrid"); if (!g.dataset.built) {
      g.innerHTML = HUBS.map(cardHTML).join(""); g.dataset.built = "1";
      HUBS.forEach(function (k, i) { g.querySelector('[data-kbd="' + k + '"]').textContent = (i + 1); });
    }
    var d = load();
    HUBS.forEach(function (k) {
      var s = d.subjects[k]; var acc = s.answered ? Math.round(s.correct / s.answered * 100) : 0;
  ...
  function renderContinue() {
    var d = load(), b = document.getElementById("continueBtn");
  ...
  function renderWeak() {
    var el = document.getElementById("weakList"); var a = weakAreas();
  ...
  function renderBadges() {
    var d = load(), grid = document.getElementById("badgeGrid");
  ...
  function renderMascot() {
    var d = load(); if (d.mascotHidden) { document.getElementById("mascot").style.display = "none"; return; }
  ...
  function renderLeaderboard() {
    var d = load(), el = document.getElementById("lbList");
    if (!d.leaderboard.length) { el.innerHTML = '<div class="empty">No scores yet — finish a quiz to make the board.</div>'; return; }
    el.innerHTML = d.leaderboard.slice(0, 8).map(function (x, i) {
      var dt = new Date(x.ts); var ds = (dt.getMonth() + 1) + "/" + dt.getDate();
      return '<div class="lb-row"><div class="lb-rank">' + (i + 1) + '</div><div class="lb-mid"><div class="t">' + (x.title || "") + '</div><div class="s">' + (META[x.subject] ? META[x.subject].name : "") + ' · ' + x.correct + '/' + x.total + ' · ' + ds + '</div></div><div class="lb-pct">' + x.pct + '%</div></div>';
    }).join("");
  }
  function applyTheme() {
    var d = load(); var lvl = rankFor(d.xp).lvl;
    if (lvl < themeMeta(d.theme).unlock) { d.theme = "dark"; save(); }
    document.documentElement.setAttribute("data-theme", d.theme || "dark");
    document.getElementById("themeBtn").textContent = themeMeta(d.theme).icon; checkGolden();
    try { pushThemeToFrames(); } catch (e) { }
  }
  function renderThemeOptions() {
    var wrap = document.getElementById("themeOptions"); if (!wrap) return;
    var d = load(); var lvl = rankFor(d.xp).lvl; var cur = d.theme || "dark";
    wrap.innerHTML = THEMES.map(function (th) {
      var locked = lvl < th.unlock, sel = cur === th.id;
      return '<button class="sync-opt theme-opt' + (sel ? ' sel' : '') + (locked ? ' locked' : '') + '" data-theme-id="' + th.id + '">' +
        '<span class="so-icon">' + (locked ? "🔒" : th.icon) + '</span>' +
        '<span class="so-text"><span class="so-t">' + th.name + (sel ? ' <span style="color:var(--accent);font-size:.7rem;font-weight:700">SELECTED</span>' : '') + '</span>' +
        '<span class="so-d">' + (locked ? ("Unlocks at Level " + th.unlock) : th.desc) + '</span></span></button>';
    }).join("");
    wrap.querySelectorAll(".theme-opt").forEach(function (b) { b.onclick = function () { setTheme(b.getAttribute("data-theme-id")); }; });
  }
  function setTheme(id) {
    var d = load(); var lvl = rankFor(d.xp).lvl; var meta = themeMeta(id);
    if (lvl < meta.unlock) { try { toast("🔒 " + meta.name + " unlocks at Level " + meta.unlock + ". Keep going!"); } catch (e) { } return; }
    var unlockingRainbow = id === "rainbow" && d.theme !== "rainbow";
    d.theme = id; save(); applyTheme(); renderThemeOptions();
    try { toast(meta.icon + " " + meta.name + " theme on"); if (unlockingRainbow) confetti(); } catch (e) { }
  }
  function pushThemeToFrames() {
    var d = load(), t = d.theme || "dark";
    ["hubFrame", "gauntletFrame"].forEach(function (id) {
      var f = document.getElementById(id); if (!f) return;
      try {
        if (f.contentWindow && f.contentWindow.GA_applyTheme) { f.contentWindow.GA_applyTheme(t); return; }
        if (f.contentWindow && f.contentWindow.document) {
          f.contentWindow.document.documentElement.setAttribute("data-ga-theme", t);
        }
      } catch (e) { }
    });
  }
  ...
  var frame, loading;
  function open(key) {
    var d0 = load(); sessionStart = { xp: d0.xp, ans: totals().ans, cor: totals().cor, ts: Date.now() }; loading.style.display = "flex";
    try {
      ...
      frame.onload = function () { loading.style.display = "none"; frame.onload = null; try { setTimeout(pushThemeToFrames, 120); setTimeout(pushThemeToFrames, 900); } catch (e) { } };
  ...
  function backToGrid() {
    frame.srcdoc = ""; frame.src = "about:blank"; document.getElementById("hubView").style.display = "none";
    ...
    if (sessionStart) {
      var T = totals(); var ans = T.ans - sessionStart.ans, cor = T.cor - sessionStart.cor, xpg = d.xp - sessionStart.xp, mins = Math.round((Date.now() - sessionStart.ts) / 60000);
  ...
  function openGauntlet() {
    ...
    if (!gauntletLoaded) {
      ...
      setTimeout(function () { try { pushThemeToFrames(); } catch (e) { } }, 800);
    }
    window.scrollTo(0, 0);
  }
  ...
  document.addEventListener("keydown", function (e) {
    ...
    cheatBuf = (cheatBuf + e.key.toLowerCase()).slice(-CHEAT_CODE.length);
    if (cheatBuf === CHEAT_CODE) { cheatBuf = ""; maxOutLevel(); }
  });
  ...
  document.addEventListener("keydown", function (e) {
    ...
    else if (e.key === "p" || e.key === "P") { setPinkTheme(); }
    else if (e.key === "r" || e.key === "R") { setRainbowTheme(); }
    else if (e.key === "m" || e.key === "M") { setMatchaTheme(); }
  });
  function toggleTheme() {
    var d = load(); var lvl = rankFor(d.xp).lvl;
    var avail = THEMES.filter(function (th) { return lvl >= th.unlock; }).map(function (th) { return th.id; });
    var i = avail.indexOf(d.theme || "dark"); setTheme(avail[(i + 1) % avail.length]);
  }
  function setPinkTheme() { var d = load(); setTheme(d.theme === "pink" ? "dark" : "pink"); }
  function setRainbowTheme() { var d = load(); setTheme(d.theme === "rainbow" ? "dark" : "rainbow"); }
  function setMatchaTheme() { var d = load(); setTheme(d.theme === "matcha" ? "dark" : "matcha"); }

  /* wire + init */
  window.GA = { open: open };
  window.backToGrid = backToGrid; window.continueLast = continueLast;
  window.closeModal = closeModal;
  window.setPinkTheme = setPinkTheme;
