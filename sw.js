...
self.addEventListener("install", function (event) {
  event.waitUntil(
    caches.open(SHELL_CACHE).then(function (cache) {
      ...
      // Activate this version as soon as it's installed instead of sitting
      // in "waiting" until someone clicks the update banner. Content fixes
      // (like corrected hub text) need to reach returning users on their
      // next visit, not depend on them noticing a banner. script.js already
      // reloads the page on controllerchange, so this is a clean one-time
      // refresh rather than a disruptive swap mid-interaction.
